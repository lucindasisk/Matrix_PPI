...
subs = pd.read_csv(home + '/LS_scripts/LS_sublist.txt',
...
def parse_runs(sub):
    if user == 'laptop':
        ...
        roimean_dir = '/mnt/abcd/derivatives/{}/LS_ABCD_368_with_GSR_LP1'.format(
            sub)

    roimean_list = glob(
        roimean_dir + '/R_{}_ses-baselineYear1Arm1_task-nBack_run-*_bold_bis_matrix_1_roimean.txt'.format(sub))
...
def run_ppi_regression(sub, run):
    ...
    roidata = pd.DataFrame(roimean).drop(
        ["ROI ", "Unnamed: 369"], axis=1).to_numpy()
    ...
    for i in range(0, roidata_zscore.shape[1] - 1):  # i is predictor node
        ...
        y_mat_nan = np.concatenate(
            (onesv, i_roi, conv_reg_matrix, int_reg_mat), axis=1)
        ...
        # j is target/outcome node
        for j in range(0, roidata_zscore.shape[1] - 1):
    ...
    pd.DataFrame(negface0b_int_array).to_csv(
        out_dir + '/{}_PPI_NegativeFace_0back_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(neutface0b_int_array).to_csv(
        out_dir + '/{}_PPI_NeutralFace_0back_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(posface0b_int_array).to_csv(
        out_dir + '/{}_PPI_PositiveFace_0back_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(place0b_int_array).to_csv(
        out_dir + '/{}_PPI_Place_0back_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(negface2b_int_array).to_csv(
        out_dir + '/{}_PPI_NegativeFace_2back_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(neutface2b_int_array).to_csv(
        out_dir + '/{}_PPI_NeutralFace_2back_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(posface2b_int_array).to_csv(
        out_dir + '/{}_PPI_PositiveFace_2back_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(place2b_int_array).to_csv(
        out_dir + '/{}_PPI_Place_2back_Run{}_{}.csv'.format(sub, run, today))
...
for sub in two_runs:
    try:
        ...
        reg_matrix_1 = pd.read_csv(
            reg_dir + '/{}_Regressor_Matrix_Block_Scan1_{}.csv'.format(sub, mat_date), header=0)
        ...
        reg_matrix_2 = pd.read_csv(
            reg_dir + '/{}_Regressor_Matrix_Block_Scan2_{}.csv'.format(sub, mat_date), header=0)
        # Create overall face regressor column by summing three face conditions
...
#         reg_matrix_2 =reg_matrix_2[['face_0back_reg', '0_back_place_scan2_reg', 'face_2back_reg', '2_back_place_scan2_reg', 'cue_scan2_reg']]
        ...
        conv_reg_matrix_1['negface_0back_reg_conv'] = convolve(
            hrf, reg_matrix_1['0_back_negface_scan1_reg'], mode='full')
        conv_reg_matrix_1['neutface_0back_reg_conv'] = convolve(
            hrf, reg_matrix_1['0_back_neutface_scan1_reg'], mode='full')
        conv_reg_matrix_1['posface_0back_reg_conv'] = convolve(
            hrf, reg_matrix_1['0_back_posface_scan1_reg'], mode='full')
        conv_reg_matrix_1['negface_2back_reg_conv'] = convolve(
            hrf, reg_matrix_1['2_back_negface_scan1_reg'], mode='full')
        conv_reg_matrix_1['neutface_2back_reg_conv'] = convolve(
            hrf, reg_matrix_1['2_back_neutface_scan1_reg'], mode='full')
        conv_reg_matrix_1['posface_2back_reg_conv'] = convolve(
            hrf, reg_matrix_1['2_back_posface_scan1_reg'], mode='full')
        conv_reg_matrix_1['place_0back_reg_conv'] = convolve(
            hrf, reg_matrix_1['0_back_place_scan1_reg'], mode='full')
        conv_reg_matrix_1['place_2back_reg_conv'] = convolve(
            hrf, reg_matrix_1['2_back_place_scan1_reg'], mode='full')
        conv_reg_matrix_1['cue_reg_conv'] = convolve(
            hrf, reg_matrix_1['cue_scan1_reg'], mode='full')

        # Do things (convolve) Scan 2
        conv_reg_matrix_2['negface_0back_reg_conv'] = convolve(
            hrf, reg_matrix_2['0_back_negface_scan2_reg'], mode='full')
        conv_reg_matrix_2['neutface_0back_reg_conv'] = convolve(
            hrf, reg_matrix_2['0_back_neutface_scan2_reg'], mode='full')
        conv_reg_matrix_2['posface_0back_reg_conv'] = convolve(
            hrf, reg_matrix_2['0_back_posface_scan2_reg'], mode='full')
        conv_reg_matrix_2['negface_2back_reg_conv'] = convolve(
            hrf, reg_matrix_2['2_back_negface_scan2_reg'], mode='full')
        conv_reg_matrix_2['neutface_2back_reg_conv'] = convolve(
            hrf, reg_matrix_2['2_back_neutface_scan2_reg'], mode='full')
        conv_reg_matrix_2['posface_2back_reg_conv'] = convolve(
            hrf, reg_matrix_2['2_back_posface_scan2_reg'], mode='full')
        conv_reg_matrix_2['place_0back_reg_conv'] = convolve(
            hrf, reg_matrix_2['0_back_place_scan2_reg'], mode='full')
        conv_reg_matrix_2['place_2back_reg_conv'] = convolve(
            hrf, reg_matrix_2['2_back_place_scan2_reg'], mode='full')
        conv_reg_matrix_2['cue_reg_conv'] = convolve(
            hrf, reg_matrix_2['cue_scan2_reg'], mode='full')
        ...
        bin_reg_matrix_1['negface_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['negface_0back_reg_conv'])
        bin_reg_matrix_1['neutface_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['neutface_0back_reg_conv'])
        bin_reg_matrix_1['posface_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['posface_0back_reg_conv'])
        bin_reg_matrix_1['negface_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['negface_2back_reg_conv'])
        bin_reg_matrix_1['neutface_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['neutface_2back_reg_conv'])
        bin_reg_matrix_1['posface_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['posface_2back_reg_conv'])
        bin_reg_matrix_1['place_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['place_0back_reg_conv'])
        bin_reg_matrix_1['place_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['place_2back_reg_conv'])
        bin_reg_matrix_1['cue_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['cue_reg_conv'])

        # Add columns to data frame with binarized data
        bin_reg_matrix_2 = pd.DataFrame()
        bin_reg_matrix_2['negface_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['negface_0back_reg_conv'])
        bin_reg_matrix_2['neutface_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['neutface_0back_reg_conv'])
        bin_reg_matrix_2['posface_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['posface_0back_reg_conv'])
        bin_reg_matrix_2['negface_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['negface_2back_reg_conv'])
        bin_reg_matrix_2['neutface_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['neutface_2back_reg_conv'])
        bin_reg_matrix_2['posface_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['posface_2back_reg_conv'])
        bin_reg_matrix_2['place_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['place_0back_reg_conv'])
        bin_reg_matrix_2['place_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['place_2back_reg_conv'])
        bin_reg_matrix_2['cue_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['cue_reg_conv'])
...
for sub in one_run:
    try:
        ...
        reg_matrix_1['face_0back_reg'] = reg_matrix_1['0_back_negface_scan1_reg'] + \
            reg_matrix_1['0_back_neutface_scan1_reg'] + \
            reg_matrix_1['0_back_posface_scan1_reg']
        reg_matrix_1['face_2back_reg'] = reg_matrix_1['2_back_negface_scan1_reg'] + \
            reg_matrix_1['2_back_neutface_scan1_reg'] + \
            reg_matrix_1['2_back_posface_scan1_reg']
        # Select only columns of interest
        reg_matrix_1 = reg_matrix_1[['face_0back_reg', '0_back_place_scan1_reg',
                                     'face_2back_reg', '2_back_place_scan1_reg', 'cue_scan1_reg']]
        ...
        conv_reg_matrix_1['face_0back_reg_conv'] = convolve(
            hrf, reg_matrix_1['face_0back_reg'], mode='full')
        conv_reg_matrix_1['place_0back_reg_conv'] = convolve(
            hrf, reg_matrix_1['0_back_place_scan1_reg'], mode='full')
        conv_reg_matrix_1['face_2back_reg_conv'] = convolve(
            hrf, reg_matrix_1['face_2back_reg'], mode='full')
        conv_reg_matrix_1['place_2back_reg_conv'] = convolve(
            hrf, reg_matrix_1['2_back_place_scan1_reg'], mode='full')
        conv_reg_matrix_1['cue_reg_conv'] = convolve(
            hrf, reg_matrix_1['cue_scan1_reg'], mode='full')
        ...
        # Add columns to data frame with binarized data
        bin_reg_matrix_1 = pd.DataFrame()
        bin_reg_matrix_1['face_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['face_0back_reg_conv'])
        bin_reg_matrix_1['place_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['place_0back_reg_conv'])
        bin_reg_matrix_1['face_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['face_2back_reg_conv'])
        bin_reg_matrix_1['place_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['place_2back_reg_conv'])
        bin_reg_matrix_1['cue_reg_conv_bin'] = binarize(
            conv_reg_matrix_1['cue_reg_conv'])
