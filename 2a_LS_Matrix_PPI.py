...
def run_ppi_regression(sub, run):
   ...
   for i in range(0, roidata_zscore.shape[1]-1): # i is predictor node
       ...
       int_reg_mat = np.array([face0b_int, place0b_int, face2b_int, place2b_int, cue_int]).transpose()   
   ...
   pd.DataFrame(int_roi_array).to_csv(out_dir +'/{}_PPI_Interceptdata_Run{}_{}.csv'.format(sub, run, today))
   pd.DataFrame(ith_roi_array).to_csv(out_dir +'/{}_PPI_ROIxROIdata_Run{}_{}.csv'.format(sub, run, today))
   pd.DataFrame(face0b_int_array).to_csv(out_dir +'/{}_PPI_face0bkdata_Run{}_{}.csv'.format(sub, run, today))
   pd.DataFrame(place0b_int_array).to_csv(out_dir +'/{}_PPI_place0bkdata_Run{}_{}.csv'.format(sub, run, today))
   pd.DataFrame(face2b_int_array).to_csv(out_dir +'/{}_PPI_face2bkdata_Run{}_{}.csv'.format(sub, run, today))
   pd.DataFrame(place2b_int_array).to_csv(out_dir +'/{}_PPI_place2bkdata_Run{}_{}.csv'.format(sub, run, today))
...
for sub in two_runs:
    try:
        ...
        conv_reg_matrix_2['face_0back_reg_conv'] = convolve(hrf, reg_matrix_2['face_0back_reg'], mode='full')
        conv_reg_matrix_2['place_0back_reg_conv'] = convolve(hrf, reg_matrix_2['0_back_place_scan2_reg'], mode='full')
        conv_reg_matrix_2['face_2back_reg_conv'] = convolve(hrf, reg_matrix_2['face_2back_reg'], mode='full')
...
#         conv_reg_matrix_1.to_csv(out_dir + '/convolved_mat2.csv')
        ...
        bin_reg_matrix_2['face_0back_reg_conv_bin'] = binarize(conv_reg_matrix_2['face_0back_reg_conv'])
        bin_reg_matrix_2['place_0back_reg_conv_bin'] = binarize(conv_reg_matrix_2['place_0back_reg_conv'])
        bin_reg_matrix_2['face_2back_reg_conv_bin'] = binarize(conv_reg_matrix_2['face_2back_reg_conv'])
