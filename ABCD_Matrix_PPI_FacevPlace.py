...
out_dir = join(home, 'LS_PPI_connectivity_reg_centered')
...
def run_ppi_regression(sub, run):
    ...
    face0b_int_array = np.empty([368, 368])
    place0b_int_array = np.empty([368, 368])
    face2b_int_array = np.empty([368, 368])
    ...
    for i in range(0, roidata_zscore.shape[1] - 1):  # i is predictor node
        ...
        face0b_int = roidata_zscore[:, i] * bin_reg_matrix[:, 0]
        place0b_int = roidata_zscore[:, i] * bin_reg_matrix[:, 1]
        face2b_int = roidata_zscore[:, i] * bin_reg_matrix[:, 2]
        place2b_int = roidata_zscore[:, i] * bin_reg_matrix[:, 3]
        cue_int = roidata_zscore[:, i] * bin_reg_matrix[:, 4]
        int_reg_mat = np.array(
            [face0b_int, place0b_int, face2b_int, place2b_int, cue_int]).transpose()
        ...
        for j in range(0, roidata_zscore.shape[1] - 1):
            ...
            ith_beta = output[6]
            face0_beta = output[7]
            place0_beta = output[8]
            face2_beta = output[9]
            place2_beta = output[10]
            cue_beta = output[11]

            # Append output to matrices
            int_roi_array[j, i] = int_beta
            ith_roi_array[j, i] = ith_beta
            face0b_int_array[j, i] = face0_beta
            place0b_int_array[j, i] = place0_beta
            face2b_int_array[j, i] = face2_beta
            place2b_int_array[j, i] = place2_beta

    # Save out data in CSV
    print('Saving data to CSV!')
    pd.DataFrame(int_roi_array).to_csv(
        out_dir + '/{}_PPI_Interceptdata_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(ith_roi_array).to_csv(
        out_dir + '/{}_PPI_ROIxROIdata_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(face0b_int_array).to_csv(
        out_dir + '/{}_PPI_face0bkdata_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(place0b_int_array).to_csv(
        out_dir + '/{}_PPI_place0bkdata_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(face2b_int_array).to_csv(
        out_dir + '/{}_PPI_face2bkdata_Run{}_{}.csv'.format(sub, run, today))
    pd.DataFrame(place2b_int_array).to_csv(
        out_dir + '/{}_PPI_place2bkdata_Run{}_{}.csv'.format(sub, run, today))
...
for sub in two_runs:
    try:
        ...
        # Create overall face regressor column by summing three face conditions
        reg_matrix_2['face_0back_reg'] = reg_matrix_2['0_back_negface_scan2_reg'] + \
            reg_matrix_2['0_back_neutface_scan2_reg'] + \
            reg_matrix_2['0_back_posface_scan2_reg']
        reg_matrix_2['face_2back_reg'] = reg_matrix_2['2_back_negface_scan2_reg'] + \
            reg_matrix_2['2_back_neutface_scan2_reg'] + \
            reg_matrix_2['2_back_posface_scan2_reg']
        # Select only columns of interest
        reg_matrix_2 = reg_matrix_2[['face_0back_reg', '0_back_place_scan2_reg',
                                     'face_2back_reg', '2_back_place_scan2_reg', 'cue_scan2_reg']]
        ...
        conv_reg_matrix_2['face_0back_reg_conv'] = convolve(
            hrf, reg_matrix_2['face_0back_reg'], mode='full')
        conv_reg_matrix_2['place_0back_reg_conv'] = convolve(
            hrf, reg_matrix_2['0_back_place_scan2_reg'], mode='full')
        conv_reg_matrix_2['face_2back_reg_conv'] = convolve(
            hrf, reg_matrix_2['face_2back_reg'], mode='full')
...
#         conv_reg_matrix_1.to_csv(out_dir + '/convolved_mat2.csv')
        ...
        bin_reg_matrix_2['face_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['face_0back_reg_conv'])
        bin_reg_matrix_2['place_0back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['place_0back_reg_conv'])
        bin_reg_matrix_2['face_2back_reg_conv_bin'] = binarize(
            conv_reg_matrix_2['face_2back_reg_conv'])
